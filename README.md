...
![Power breakdown — 50 MHz vs 100 MHz](docs/power_breakdown.png)

| Component | 50 MHz (baseline) | 100 MHz (aggressive) | Δ |
|---|--:|--:|--:|
| internal | 1.921 mW | 3.731 mW | +94% |
| switching | 0.676 mW | 1.295 mW | +92% |
| leakage | ~0 mW | ~0 mW | — |
| **total** | **2.597 mW** | **5.026 mW** | **+94%** |

Both components roughly double at 100 MHz. The split is consistent
across corners: ~74 % internal (cell output transitions charging internal
capacitances) and ~26 % switching (net capacitances). Leakage is
negligible on Sky130A at 25 °C.

The near-linear scaling with frequency is expected for a fully
combinational-style design with no clock gating — every flop in the
DFS stack and FSM toggles every cycle regardless of whether the solver
is actively computing. Adding ICG (integrated clock gate) cells on the
stack registers would be the first cut at reducing the 94 % increase;
at the 100 MHz density (80 %) there is enough die area to absorb the
extra cells.

Generated by `power_breakdown.py` from the `power__*__total` fields in
LibreLane's `metrics.json` (populated by OpenSTA `report_power` during
the sign-off step).
...
The CI `gl_test` job (part of the `gds` workflow) runs the same cocotb
regression against the post-synthesis netlist, not the RTL. It passes on
every push, providing a simulation-level sanity check that complements
the Yosys equivalence proof in `formal/`.

To reproduce locally (requires Sky130A PDK):

```bash
bash test/setup_gl_sim.sh          # copies nl_baseline_clean.v → test/gate_level_netlist.v
PDK_ROOT=/path/to/sky130A-parent \
  make -C test GATES=yes           # icarus + cocotb against the post-synth netlist
```

`GATES=yes` compiles with `-DFUNCTIONAL -DUNIT_DELAY=#1`, so the sim uses
functional models (no SDF back-annotation). The test vectors are identical
to the RTL regression; any logic difference introduced by synthesis would
show up here before sign-off.
